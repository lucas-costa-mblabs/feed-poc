 ...
 */
export const resolveGoTemplate = (
  template: string | undefined,
  dataContext: Record<string, any> | undefined,
): string => {
  ...
  const stack: { isDisplaying: boolean; hasExecuted: boolean }[] = [];
  const isCurrentlyDisplaying = () => {
    for (const s of stack) {
      if (!s.isDisplaying) return false;
    }
    return true;
  };
  ...
  const evaluateValue = (val: string): any => {
    ...
    if (!cleanPath) return dataContext;
    ...
    if (token.startsWith("{{") && token.endsWith("}}")) {
      ...
      if (content.startsWith("if ")) {
        ...
        if (prev.hasExecuted) {
          ...
          if (prev.isDisplaying) prev.hasExecuted = true;
        }
